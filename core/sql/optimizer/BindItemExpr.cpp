...
#pragma warn(1506)  // warning elimination
  ...
  #pragma warn(1506)  // warning elimination
	...
	child1 = new (bindWA->wHeap()) Upper(child1);
      ...
      // length of prefix is longer than what we can get goos stats for
      // later we will collapse histogram into one interval if this flag is set, 
      // It is better to not flag this BiRelat as not originating from like so that 
      // we get a better histogram on it. We may lose some knowledge of correlation between
      // begin/end keys but it is better to have 2 unrelated birelats with good stats than 
      // correlated begin/end preds with a single interval histogram that is out of
      // matching length for the predicate. JIRA 2512
      if(boundExpr || prefix.length() < 7)
